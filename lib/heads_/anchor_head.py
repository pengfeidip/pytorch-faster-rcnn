...
class AnchorHead(nn.Module):
    ...
    def single_image_targets(self, level_cls_outs, level_reg_outs, gt_bbox, gt_label,
        ...
        logging.debug(' in single image targets '.center(50, '*'))
    ...
    def calc_loss(self, tar_cls_out, tar_reg_out, tar_labels, tar_param, train_cfg):
        logging.info('calculate loss of combined images')
        ...
        logging.info('avg_factor: {}'.format(avg_factor))
    ...
    # cfg contain assigner, sampler, allowed_border
    def loss(self, cls_outs, reg_outs, gt_bboxes, gt_labels, img_metas, train_cfg):
        logging.info(' find targets and calculate loss '.center(50, '='))
        ...
        img_tar_cls_out, img_tar_reg_out, img_tar_labels, img_tar_param = [], [], [], []
        for i, gt_info in enumerate(zip(gt_bboxes, gt_labels, img_metas)):
            ...
            tar_cls_out, tar_reg_out, tar_labels, tar_param \
            ...
            img_tar_labels.append(tar_labels)
        ...
        img_tar_labels  = torch.cat(img_tar_labels)
        img_tar_param   = torch.cat(img_tar_param, dim=1)

        logging.info('tar_cls_out of all images: {}'.format(img_tar_cls_out.shape))
        logging.info('tar_labels if all images: {}'.format(img_tar_labels.shape))

        return self.calc_loss(img_tar_cls_out, img_tar_reg_out, img_tar_labels, img_tar_param, train_cfg)
